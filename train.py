...
# data = "tests/taylorswift.txt"   # <- a copy of the wikipedia article on Taylor Swift

for TokenizerClass, name in zip([QuickTokenizer, BatchTokenizer], ['quick', 'batch']):
    t0 = time.time()
    tokenizer = TokenizerClass()
    tokenizer.train(data, 10000, verbose=True)   # the more merges you do, the larger the average batch size will be
    t1 = time.time()
